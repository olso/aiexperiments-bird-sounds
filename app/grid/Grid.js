...
var Grid = (module.exports = function() {
  ...
  this.createListeners = function() {
    ...
    var onDragging = function(event) {
      ...
      if (touchState === IS_ZOOMING) {
        ...
        var dy = touches[0].clientY - touches[1].clientY;
        pinchDistanceSquared = dx * dx + dy * dy;
        this.zoom(pinchDistanceSquaredStart < pinchDistanceSquared, 0.025);
        return;
      }
    }.bind(scope);
    ...
    var onButtonLoop = function(event) {
      ...
      var isInRange =
        this.isInBounds(birdGrid.cursor.x, 0, this.sprite.storedWidth) &&
        this.isInBounds(birdGrid.cursor.y, 0, this.sprite.storedHeight);
      ...
      if (
        touchState === IS_SETTLING &&
        Math.abs(speed.x) < 0.1 &&
        Math.abs(speed.y) < 0.1
      ) {
      ...
      if (isGridVisible) {
        birdGrid.x +=
          ((window.innerWidth - birdGrid.width) * 0.5 - birdGrid.x) * 0.1;
        birdGrid.y +=
          ((window.innerHeight - birdGrid.height) * 0.5 - birdGrid.y) * 0.1;
      } else {
        birdGrid.x -= speed.x;
        birdGrid.y -= speed.y;
      }
      ...
      if (Data.filteredList.length !== 0 && Data.filteredList.length <= 5000) {
        ...
        birdGrid.cursor.x =
          (smallestIndex % Config.gridTotalX) * Config.gridUnit;
        birdGrid.cursor.y =
          ((smallestIndex / Config.gridTotalX) | 0) * Config.gridUnit;
      }
    ...
    document.body.addEventListener(
      ...
      function(event) {
        ...
        switch (event.keyCode) {
          case 38: // UP
            birdGrid.y =
              normal.y < -bounds.y ? birdGrid.y + increment.y : birdGrid.y;
            cursorIndex.y = this.clamp(
              --cursorIndex.y,
              0,
              Config.gridTotalY - 1
            );
          ...
          case 37: // LEFT
            birdGrid.x =
              normal.x < -bounds.x ? birdGrid.x + increment.x : birdGrid.x;
            cursorIndex.x = this.clamp(
              --cursorIndex.x,
              0,
              Config.gridTotalX - 1
            );
  ...
  this.setBird = function(bird) {
    ...
    var soundOffsetIndex =
      (index + Config.gridTotalX) % (Config.gridTotalX * Config.gridTotalY);
    var selectedBirdData = Data.getBird(index);
    scope.birdData = Data.getBird(soundOffsetIndex);

    // visuals
    birdGrid.cursorSnapped.x = scope.snapToGrid(
      bird.x * Config.gridUnit,
      Config.gridUnit
    );
    birdGrid.cursorSnapped.y = scope.snapToGrid(
      bird.y * Config.gridUnit,
      Config.gridUnit
    );
  ...
  this.zoom = function(isZoomIn, zoomStrength) {
    ...
    if (
      !isZoomIn &&
      birdGrid.scale.x <= (windowDistance - margin * 2) / spriteDistance
    ) {
      birdGrid.x +=
        ((window.innerWidth - birdGrid.width) * 0.5 - birdGrid.x) * 0.2;
      birdGrid.y +=
        ((window.innerHeight - birdGrid.height) * 0.5 - birdGrid.y) * 0.2;
    ...
    var beforeTransform = scope.getGraphCoordinates(
      dragger.container.x,
      dragger.container.y
    );
    birdGrid.updateTransform();
    var afterTransform = scope.getGraphCoordinates(
      dragger.container.x,
      dragger.container.y
    );

    birdGrid.position.x +=
      (afterTransform.x - beforeTransform.x) * birdGrid.scale.x;
    birdGrid.position.y +=
      (afterTransform.y - beforeTransform.y) * birdGrid.scale.y;
    birdGrid.updateTransform();
  ...
  this.filter = function() {
    ...
    for (var i = 0; i < totalSprites; i++) {
      var soundOffsetIndex =
        (i + Config.gridTotalX) % (Config.gridTotalX * Config.gridTotalY);
      if (
        Data.filterStates[Data.getBirdIndex(soundOffsetIndex)] ===
        Config.filterHidden
      ) {
        filterCovers.children[i].y =
          ((i / Config.gridTotalX) | 0) * Config.gridUnit;
      } else {
